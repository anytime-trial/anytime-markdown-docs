...
| 保存 | Web | 両方 | x | o | o | o | [U:EditorToolbar](../../packages/editor-core/src/__tests__/EditorToolbar.test.tsx) | 2026-03-15 | - | | `supportsDirectAccess` 時のみ表示。Readonly 時は無効。VS は VSCode ネイティブ |
| 名前を付けて保存 | Web | 両方 | x | o | o | o | [U:EditorToolbar](../../packages/editor-core/src/__tests__/EditorToolbar.test.tsx) | 2026-03-15 | [E:file-ops](../../packages/web-app/e2e/file-ops.spec.ts) | 2026-03-15 | Readonly 時は無効。VS は VSCode ネイティブ |
...
| スラッシュコマンド | 全 | 両方 | x | x | o | o | - | | [E:toolbar](../../packages/web-app/e2e/toolbar.spec.ts) | 2026-03-15 | `editable` 状態に依存。結果 0 件時は「一致するコマンドがありません」を表示 |
...
| 右クリックコンテキストメニュー | 全 | 両方 | △ | △ | o | x | - | | manual | | 切り取り/コピー/貼り付け/Markdownで貼り付け/コードブロックで貼り付け。Readonly/Review 時はコピーのみ有効。ショートカットキー表示付き |
| ブロック要素のコピー/カット | 全 | 両方 | x | x | o | x | - | | manual | | Ctrl+C/X でブロック全体をコピー/カット。Ctrl+V またはポップアップメニューの貼り付けでブロック構造を保持して復元。`blockClipboard.ts` で共通化 |
| ブロック移動（Alt+Arrow） | 全 | 両方 | x | x | o | x | [U:keyboard](../../packages/editor-core/src/__tests__/keyboardShortcuts.test.ts) | 2026-03-18 | [E:keyboard](../../packages/web-app/e2e/keyboard-shortcuts.spec.ts) | 2026-03-18 | Alt+Up/Down でカーソル位置のブロックを上下に移動 |
| ブロック複製（Shift+Alt+Arrow） | 全 | 両方 | x | x | o | x | [U:keyboard](../../packages/editor-core/src/__tests__/keyboardShortcuts.test.ts) | 2026-03-18 | [E:keyboard](../../packages/web-app/e2e/keyboard-shortcuts.spec.ts) | 2026-03-18 | Shift+Alt+Up/Down でカーソル位置のブロックを上下に複製 |
| 空行挿入（Ctrl+Enter） | 全 | 両方 | x | x | o | x | [U:keyboard](../../packages/editor-core/src/__tests__/keyboardShortcuts.test.ts) | 2026-03-18 | - | | Ctrl+Enter で下に空行、Ctrl+Shift+Enter で上に空行 |
| 行選択（Ctrl+L） | 全 | 両方 | x | x | o | x | [U:keyboard](../../packages/editor-core/src/__tests__/keyboardShortcuts.test.ts) | 2026-03-18 | - | | カーソル位置のブロック全体を選択 |
| 単語選択（Ctrl+D） | 全 | 両方 | x | x | o | x | [U:keyboard](../../packages/editor-core/src/__tests__/keyboardShortcuts.test.ts) | 2026-03-18 | - | | カーソル位置の単語を選択（日本語対応） |
| 見出しレベル変更（Tab/Shift+Tab） | 全 | 両方 | x | x | o | x | - | | manual | | 見出し内で Tab: レベルを下げる（H1→H2）、Shift+Tab: 上げる（H2→H1） |
| 罫線テーブル自動変換 | 全 | 両方 | x | x | o | x | [U:boxTable](../../packages/editor-core/src/__tests__/boxTableToMarkdown.test.ts) | 2026-03-18 | - | | Unicode Box Drawing テーブルを貼り付け時に Markdown テーブルに自動変換 |
| Base64 画像データ折りたたみ | 全 | 両方 | o | o | o | o | [U:base64Collapse](../../packages/editor-core/src/__tests__/base64Collapse.test.ts) | 2026-03-21 | - | | Source モードで base64 画像を短いトークン（`data:base64-image-N`）に置換して表示。元データは state に保持され保存・コピー時に復元。トークン部分はバッジ風背景色で視覚的に区別 |
...
| チェックボックス操作 | 全 | 両方 | x | o | o | x | - | | [E:text-formatting](../../packages/web-app/e2e/text-formatting.spec.ts) | 2026-03-15 | Readonly は CSS `pointer-events: none`。比較モード左側は DOM キャプチャで操作無効化 |
...
対象ブロック: コードブロック / テーブル / 画像 / ダイアグラム（Mermaid / PlantUML） / 数式（Math） / HTML / GIF
...
| 画像 D&D 挿入 | 全 | 両方 | x | x | o | x | 画像 | manual | | manual | | 画像ファイルをエディタにドラッグ&ドロップで挿入。VS Code では images/ に自動保存しリンク挿入 |
| 画像クリップボード貼り付け | 全 | 両方 | x | x | o | x | 画像 | manual | | manual | | Ctrl+V でクリップボードから画像を貼り付け。VS Code では images/ に自動保存しリンク挿入 |
| Excel/Google Sheets 表の貼り付け | 全 | 両方 | x | x | o | x | テーブル | manual | | manual | | スプレッドシートからコピーした表をテーブルとして貼り付け。セル内改行は `<br>` に変換 |
| ブロック削除 | 全 | 両方 | x | x | o | x | 全ブロック | - | | [E:block-elements](../../packages/web-app/e2e/block-elements.spec.ts) | 2026-03-15 | 比較モード左側では非表示 |
| ドラッグハンドル | 全 | 両方 | x | x | o | x | 全ブロック | manual | | manual | | ドラッグで位置を移動。比較モード左側: レビューモード時は非表示、編集モード時はラベルのみ表示（選択時） |
...
| 画像エクスポート | 全 | 両方 | o | o | o | x | 画像 / ダイアグラム / GIF | manual | | manual | | ハンドルバーのカメラアイコンからブロックを PNG/SVG/GIF として保存。`showSaveFilePicker` でファイル名・保存先を指定可能 |
| 画像コメント（アノテーション） | 全 | 両方 | o | o | o | x | 画像 | manual | | manual | | SVG オーバーレイで矩形/円/線を描画しコメントを追加。解決/削除対応。コメントパネルに統合表示。Markdown 末尾の `<!-- image-comments -->` ブロックで永続化 |
| 画像トリミング | 全 | 両方 | x | x | o | x | 画像 | manual | | manual | | 編集画面でドラッグ範囲選択しトリミング。Base64/リンク画像の分岐保存対応 |
| 画像リサイズ | 全 | 両方 | x | x | o | x | 画像 | manual | | manual | | プリセットボタン（25%/50%/75%/100%/150%/200%）による倍率変更 |
| 画像ルーラー・グリッド | 全 | 両方 | o | o | o | x | 画像 | manual | | manual | | 編集画面にピクセル目盛りのルーラーとグリッド線を表示 |
| GIF 録画 | Web | 両方 | x | x | o | x | GIF | manual | | manual | | getDisplayMedia で画面キャプチャ → 矩形選択 → 録画 → GIF エンコード。最大30秒、10FPS。VS Code webview では getDisplayMedia 不可 |
| GIF 再生コントロール | 全 | 両方 | o | o | o | x | GIF | manual | | manual | | 再生/一時停止。インラインプレビュー表示 |
| スクリーンキャプチャ | Web | 両方 | x | x | o | x | 画像 | manual | | manual | | `getDisplayMedia` で画面選択 → プレビュー → キャプチャ → トリミング → 画像ブロックとして挿入。VS Code webview では `getDisplayMedia` 不可 |
...
| 画像 | `EditDialogWrapper` | Alt/Title 編集 + トリミング + リサイズ + ルーラー/グリッド + アノテーション |
| GIF | `GifPlayerDialog` | GIF プレビュー + 再生情報（FPS/duration/width）表示 |
...
| スクリーンショット（PNG） | 全 | 両方 | o | o | o | x | manual | | manual | | インラインのハンドルバーからキャプチャ。`showSaveFilePicker` でファイル名・保存先を指定 |
...
| トリミング | 全 | 両方 | x | x | o | x | manual | | manual | | ドラッグで範囲選択。Base64 画像は Base64 で保存、リンク画像はファイル上書き |
| リサイズ（倍率指定） | 全 | 両方 | x | x | o | x | manual | | manual | | プリセットボタン: 25% / 50% / 75% / 100% / 150% / 200% |
| ルーラー表示 | 全 | 両方 | o | o | o | x | manual | | manual | | ピクセル目盛りのルーラーを画像の上端・左端に表示 |
| グリッド線表示 | 全 | 両方 | o | o | o | x | manual | | manual | | 半透明のガイドグリッド線を表示 |
| アノテーション | 全 | 両方 | x | x | o | x | manual | | manual | | SVG オーバーレイで矩形/円/線 + コメント。6色パレット。解決/削除対応 |
...
| 用紙サイズ | 全 | 両方 | o | o | o | o | [U:calcPaperContentWidth](../../packages/editor-core/src/__tests__/calcPaperContentWidth.test.ts) | 2026-03-20 | - | | A3 / A4 / B4 / B5 / OFF。エディタ幅を用紙サイズに合わせて制限。96 DPI 換算 |
| 用紙余白 | 全 | 両方 | o | o | o | o | [U:calcPaperContentWidth](../../packages/editor-core/src/__tests__/calcPaperContentWidth.test.ts) | 2026-03-20 | - | | 10〜40mm（5mm 刻み、デフォルト 20mm）。用紙サイズ選択時のみ有効 |
...
| セマンティック比較 | PC/VS | 比較 | o | o | o | o | [U:semanticDiff](../../packages/editor-core/src/__tests__/semanticDiff.test.ts), [U:sectionParser](../../packages/editor-core/src/__tests__/sectionParser.test.ts) | 2026-03-18 | - | | 見出しベースのセクションマッチング。トグルボタンで行ベース/セマンティック切替。見出しなし時は行ベースにフォールバック |
...
| Error Boundary | 全 | 両方 | o | o | o | o | - | | - | | コンポーネントエラー時にクラッシュせず、エラーメッセージ + 再読み込みボタンを表示 |
...
| 機能 | 説明 |
| --- | --- |
| activationEvents 最適化 | `onLanguage:markdown` + `onView` で起動。不要な拡張アクティベーションを回避 |
| Workspace Trust | `untrustedWorkspaces: limited`。信頼されないワークスペースでもプレビュー機能は利用可能 |
| リンク検証 | Diagnostics API でファイル存在・アンカー存在チェック。問題パネルに表示 |
| クリップボード画像自動保存 | Ctrl+V / D&D で画像を images/ フォルダにファイル保存しリンク挿入 |
| パスのコピー | ツリービュー右クリック → ファイル/フォルダのフルパスをクリップボードにコピー |
| ファイルインポート | ツリービュー右クリック → ファイル選択ダイアログで外部ファイルをフォルダにコピー（複数選択対応） |
| 外部ファイル D&D | VS Code エクスプローラからファイルをドラッグ&ドロップでコピー（`text/uri-list`） |
| 外部変更の自動再読み込み | ツールバーのトグルボタン（Sync/SyncDisabled アイコン）で ON/OFF。有効時は外部ファイル変更を通知ダイアログなしで即座にエディタに反映 |
...
| 保存 | Web | 両方 | x | △ | △ | △ | [U:EditorToolbar](../../packages/editor-core/src/__tests__/EditorToolbar.test.tsx) | 2026-03-15 | - | | `hasFileHandle` 時のみ有効。Readonly 時は無効。`externalSaveOnly` 時は PC のみ表示 |
...
| アイコン | 対応 | 比較 | Readonly | Review | WYSIWYG | Source | UT | UT確認日 | E2E | E2E確認日 | 備考 |
| --- | :---: | :---: | :---: | :---: | :---: | :---: | --- | --- | --- | --- | --- |
| 再読み込み | VS | 両方 | o | o | o | o | manual | | manual | | `onReload` 提供時のみ表示。外部変更を手動でエディタに反映 |
| 自動再読み込みトグル | VS | 両方 | o | o | o | o | manual | | manual | | Sync/SyncDisabled アイコン。ON 時は外部変更を通知なしで即時反映 |
...
| パスのコピー | PC | 右クリックメニューからファイル/フォルダのフルパスをクリップボードにコピー |
| ファイルインポート | PC | 右クリックメニューからファイル選択ダイアログで外部ファイルをフォルダにコピー（複数選択対応） |
| VS Code エクスプローラからの D&D コピー | VS | VS Code エクスプローラからファイルをドラッグ&ドロップでコピー（`text/uri-list`） |
...
| `NEXT_PUBLIC_SITE_URL` | サイト URL | `https://www.anytime-trial.com` |
